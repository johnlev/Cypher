...
@synthesize button,label,encrypt,key,choice,clear,passes;
...
-(IBAction)clicked:(id)sender{
    ...
    printf("[");
    [self.view endEditing:YES];
    while (passed < passes.selectedSegmentIndex + 1) {
        if (choice.selectedSegmentIndex == 0) {
            ...
            if (passed == 0){
                ...
                for (unsigned int i = 0; i < length; i++) {
                    input[i] = [label.text characterAtIndex:i];
                }
            }
            for (int b=0; b<[encrypt.text length];b++) {
                if (input[b] == '\n') {
                    input[b] = ' ';
                }
            }
